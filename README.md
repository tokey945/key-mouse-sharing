...
- 局域网发现：通过 UDP 广播自动展示在线设备；点击连接后由目标设备确认，确认后自动建立键鼠连接。
...
1. 两台设备连接到同一局域网。
2. 两端打开应用后，在“键鼠共享”的局域网设备列表中选择目标设备并点击“连接”。
3. 目标设备点击“允许一次”或“允许并记住”，应用会自动启动接收监听并建立连接。
4. 如果 UDP 广播被路由器或防火墙拦截，仍可在设置页填写目标 IP，再使用控制端/客户端手动连接。
5. 文件接收目录未配置时不影响键鼠连接，但文件接收功能不会启动。
...
- 自动发现使用 UDP `47821` 端口，键鼠连接默认使用 TCP `4000`；防火墙需要允许应用在专用网络通信。
